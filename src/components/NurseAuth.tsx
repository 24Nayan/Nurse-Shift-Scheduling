...
const NurseAuth = () => {
  const auth = useAuth() as {
    ...
    signup?: (nurseId: string, password: string) => Promise<any>;
    isLoading?: boolean;
    error?: string | null;
    clearError?: () => void;
  } || {};

  const login = auth.login ?? (() => Promise.reject('login not implemented'));
  const signup = auth.signup ?? (() => Promise.reject('signup not implemented'));
  ...
  const handleSignup = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await signup(signupData.nurseId, signupData.password);
      // Redirect will be handled by the app router
    } catch (error) {
              ...
              <CardContent>
                <form onSubmit={handleLogin} className="space-y-4">
                  <div className="space-y-2">
                    <Label htmlFor="login-nurseId">Nurse ID</Label>
                    <Input
                      id="login-nurseId"
                      name="nurseId"
                      type="text"
                      ...
                      className="uppercase"
                      pattern="N\d{4}"
                      title="Please enter a valid Nurse ID (e.g., N0001)"
                    />
                  </div>
